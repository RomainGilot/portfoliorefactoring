...
import { Bars3Icon, DocumentDuplicateIcon, FolderIcon, HomeIcon, XMarkIcon } from "@heroicons/react/24/outline";
...
const navigation = [
  ...
  { name: "Traductions", href: "#", icon: DocumentDuplicateIcon, current: false },
];
            ...
            <div className="flex justify-end flex-1 gap-x-4 self-stretch lg:gap-x-6">
              <div className="flex items-center gap-x-4 lg:gap-x-6">
                <Menu as="div" className="relative">
                  <MenuButton className="flex -m-1.5 flex items-center p-1.5">
                  ...
                  >
                    {userNavigation.map((item) => (
                      <MenuItem key={item.name}>
                        <button onClick={handleLogout} className="block w-full text-left px-3 py-1 text-sm leading-6 text-gray-900 data-[focus]:bg-gray-50">
                          {item.name}
                        </button>
                      </MenuItem>
                    ))}
